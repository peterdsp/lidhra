  ...
  if multiple_scenes_enabled() {
    ...
    if let Some(scene) = app_state::unitialized_scene() {
      ...
      unsafe {
        ...
        // While a scene is actively connecting (app launch), `connect_scene`
        // attaches this just-registered window to that scene, so requesting a new
        // scene here is not only redundant, it churns the main scene and drops the
        // app to the background (a black screen on iPhone / iPhone Duo). Only
        // request a new scene for windows created outside a scene connection.
        if !app_state::is_connecting_scene() {
