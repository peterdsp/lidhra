...
struct AppState {
  ...
  // True while a scene is actively connecting (inside `connect_scene`, around
  // `on_app_ready`). A window created during that window is attached to the
  // connecting scene by `connect_scene` itself, so `create_window` must not also
  // request a brand new scene: on iPhone (incl. Duo) that extra request churns
  // the main scene and it deactivates to the background, leaving a black screen.
  connecting_scene: bool,
}
...
pub unsafe fn connect_scene(scene: &UIScene, options: &UISceneConnectionOptions) {
  ...
  if !did_first_scene_connect {
    ...
    // A window created inside `on_app_ready` belongs to this connecting scene and
    // is attached below; `create_window` must not request another scene for it.
    AppState::get_mut().connecting_scene = true;
    on_app_ready();
    AppState::get_mut().connecting_scene = false;
  }
  ...
  if let Some(window_scene) = scene.downcast_ref::<UIWindowScene>() {
    ...
    if let Some(window) = window {
      ...
      // Attaching the window to the scene does not show it; without this the
      // window stays hidden and the screen is black on iOS 27 (iPhone / Duo).
...
// True while a scene is actively connecting; a window created now is attached to
// that scene by `connect_scene`, so `create_window` should not request a new one.
pub unsafe fn is_connecting_scene() -> bool {
  AppState::get_mut().connecting_scene
}
